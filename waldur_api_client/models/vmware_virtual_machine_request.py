...
class VmwareVirtualMachineRequest:
    ...
    def to_multipart(self) -> dict[str, Any]:
        ...
        guest_os: Union[Unset, tuple[None, bytes, str]]

        if isinstance(self.guest_os, Unset):
            guest_os = UNSET
        elif isinstance(self.guest_os, GuestOsEnum):
            guest_os = (None, str(self.guest_os.value).encode(), "text/plain")
        elif isinstance(self.guest_os, None):
            guest_os = (None, str(self.guest_os).encode(), "text/plain")
        else:
            guest_os = (None, str(self.guest_os).encode(), "text/plain")
        ...
        networks: Union[Unset, tuple[None, bytes, str]] = UNSET
        if not isinstance(self.networks, Unset):
            _temp_networks = []
            for networks_item_data in self.networks:
                networks_item = networks_item_data.to_dict()
                _temp_networks.append(networks_item)
            networks = (None, json.dumps(_temp_networks).encode(), "application/json")

        datastore: Union[Unset, tuple[None, bytes, str]]

        if isinstance(self.datastore, Unset):
            datastore = UNSET
        elif isinstance(self.datastore, str):
            datastore = (None, str(self.datastore).encode(), "text/plain")
        else:
            datastore = (None, str(self.datastore).encode(), "text/plain")

        folder: Union[Unset, tuple[None, bytes, str]]
