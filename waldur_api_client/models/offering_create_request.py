...
class OfferingCreateRequest:
    ...
    def to_multipart(self) -> dict[str, Any]:
        ...
        full_description = (
            ...
            if isinstance(self.full_description, Unset)
            else (None, str(self.full_description).encode(), "text/plain")
        )

        terms_of_service = (
            self.terms_of_service
            if isinstance(self.terms_of_service, Unset)
            else (None, str(self.terms_of_service).encode(), "text/plain")
        )

        terms_of_service_link = (
            self.terms_of_service_link
            if isinstance(self.terms_of_service_link, Unset)
            else (None, str(self.terms_of_service_link).encode(), "text/plain")
        )

        privacy_policy_link = (
            self.privacy_policy_link
            if isinstance(self.privacy_policy_link, Unset)
            else (None, str(self.privacy_policy_link).encode(), "text/plain")
        )

        access_url = (
            self.access_url
            if isinstance(self.access_url, Unset)
            else (None, str(self.access_url).encode(), "text/plain")
        )
        ...
        if not isinstance(self.options, Unset):
            options = (None, json.dumps(self.options.to_dict()).encode(), "application/json")

        resource_options: Union[Unset, tuple[None, bytes, str]] = UNSET
        if not isinstance(self.resource_options, Unset):
        ...
        if not isinstance(self.components, Unset):
            ...
            for components_item_data in self.components:
                components_item = components_item_data.to_dict()
                _temp_components.append(components_item)
            components = (None, json.dumps(_temp_components).encode(), "application/json")

        vendor_details = (
            self.vendor_details
            if isinstance(self.vendor_details, Unset)
            else (None, str(self.vendor_details).encode(), "text/plain")
        )

        getting_started = (
            self.getting_started
            if isinstance(self.getting_started, Unset)
            else (None, str(self.getting_started).encode(), "text/plain")
        )

        integration_guide = (
            self.integration_guide
            if isinstance(self.integration_guide, Unset)
            else (None, str(self.integration_guide).encode(), "text/plain")
        )

        thumbnail: Union[Unset, tuple[None, bytes, str]]
        ...
        if isinstance(self.thumbnail, Unset):
            ...
            thumbnail = (None, str(self.thumbnail).encode(), "text/plain")

        plans: Union[Unset, tuple[None, bytes, str]] = UNSET
        if not isinstance(self.plans, Unset):
            _temp_plans = []
            for plans_item_data in self.plans:
                plans_item = plans_item_data.to_dict()
                _temp_plans.append(plans_item)
            plans = (None, json.dumps(_temp_plans).encode(), "application/json")

        shared = self.shared if isinstance(self.shared, Unset) else (None, str(self.shared).encode(), "text/plain")

        billable = (
            self.billable if isinstance(self.billable, Unset) else (None, str(self.billable).encode(), "text/plain")
        )

        datacite_doi = (
            self.datacite_doi
            if isinstance(self.datacite_doi, Unset)
            else (None, str(self.datacite_doi).encode(), "text/plain")
        )
        ...
        backend_metadata = (
            ...
            if isinstance(self.backend_metadata, Unset)
            else (None, str(self.backend_metadata).encode(), "text/plain")
        )
