from collections.abc import Mapping
from typing import Any, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="ResourceRestrictMemberAccessRequest")


@_attrs_define
class ResourceRestrictMemberAccessRequest:
    """
    Attributes:
        restrict_member_access (Union[Unset, bool]):
    """

    restrict_member_access: Union[Unset, bool] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        restrict_member_access = self.restrict_member_access

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if restrict_member_access is not UNSET:
            field_dict["restrict_member_access"] = restrict_member_access

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        restrict_member_access = d.pop("restrict_member_access", UNSET)

        resource_restrict_member_access_request = cls(
            restrict_member_access=restrict_member_access,
        )

        resource_restrict_member_access_request.additional_properties = d
        return resource_restrict_member_access_request

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
