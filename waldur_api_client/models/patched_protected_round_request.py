...
@_attrs_define
class PatchedProtectedRoundRequest:
    """
    Attributes:
    ...
    def to_multipart(self) -> dict[str, Any]:
        start_time: Union[Unset, bytes] = UNSET
        if not isinstance(self.start_time, Unset):
            start_time = self.start_time.isoformat().encode()

        cutoff_time: Union[Unset, bytes] = UNSET
        if not isinstance(self.cutoff_time, Unset):
            cutoff_time = self.cutoff_time.isoformat().encode()

        review_strategy: Union[Unset, tuple[None, bytes, str]] = UNSET
        if not isinstance(self.review_strategy, Unset):
            review_strategy = (None, str(self.review_strategy.value).encode(), "text/plain")

        deciding_entity: Union[Unset, tuple[None, bytes, str]] = UNSET
        if not isinstance(self.deciding_entity, Unset):
            deciding_entity = (None, str(self.deciding_entity.value).encode(), "text/plain")

        allocation_time: Union[Unset, tuple[None, bytes, str]] = UNSET
        if not isinstance(self.allocation_time, Unset):
            allocation_time = (None, str(self.allocation_time.value).encode(), "text/plain")

        allocation_date: Union[Unset, tuple[None, bytes, str]]

        if isinstance(self.allocation_date, Unset):
            allocation_date = UNSET
        elif isinstance(self.allocation_date, datetime.datetime):
            allocation_date = self.allocation_date.isoformat().encode()
        else:
            allocation_date = (None, str(self.allocation_date).encode(), "text/plain")

        minimal_average_scoring: Union[Unset, tuple[None, bytes, str]]

        if isinstance(self.minimal_average_scoring, Unset):
            minimal_average_scoring = UNSET
        elif isinstance(self.minimal_average_scoring, str):
            minimal_average_scoring = (None, str(self.minimal_average_scoring).encode(), "text/plain")
        else:
            minimal_average_scoring = (None, str(self.minimal_average_scoring).encode(), "text/plain")

        review_duration_in_days = (
            self.review_duration_in_days
            if isinstance(self.review_duration_in_days, Unset)
            else (None, str(self.review_duration_in_days).encode(), "text/plain")
        )

        minimum_number_of_reviewers: Union[Unset, tuple[None, bytes, str]]

        if isinstance(self.minimum_number_of_reviewers, Unset):
            minimum_number_of_reviewers = UNSET
        elif isinstance(self.minimum_number_of_reviewers, int):
            minimum_number_of_reviewers = (None, str(self.minimum_number_of_reviewers).encode(), "text/plain")
        else:
            minimum_number_of_reviewers = (None, str(self.minimum_number_of_reviewers).encode(), "text/plain")
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        return patched_protected_round_request
