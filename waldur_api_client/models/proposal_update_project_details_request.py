...
class ProposalUpdateProjectDetailsRequest:
    ...
    def to_multipart(self) -> dict[str, Any]:
        ...
        project_summary = (
            ...
            else (None, str(self.project_summary).encode(), "text/plain")
        )

        project_is_confidential = (
            self.project_is_confidential
            if isinstance(self.project_is_confidential, Unset)
            else (None, str(self.project_is_confidential).encode(), "text/plain")
        )

        project_has_civilian_purpose = (
            self.project_has_civilian_purpose
            if isinstance(self.project_has_civilian_purpose, Unset)
            else (None, str(self.project_has_civilian_purpose).encode(), "text/plain")
        )

        duration_in_days: Union[Unset, tuple[None, bytes, str]]

        if isinstance(self.duration_in_days, Unset):
            duration_in_days = UNSET
        elif isinstance(self.duration_in_days, int):
            duration_in_days = (None, str(self.duration_in_days).encode(), "text/plain")
        else:
            duration_in_days = (None, str(self.duration_in_days).encode(), "text/plain")

        oecd_fos_2007_code: Union[Unset, tuple[None, bytes, str]]

        if isinstance(self.oecd_fos_2007_code, Unset):
            oecd_fos_2007_code = UNSET
        elif isinstance(self.oecd_fos_2007_code, OecdFos2007CodeEnum):
            oecd_fos_2007_code = (None, str(self.oecd_fos_2007_code.value).encode(), "text/plain")
        elif isinstance(self.oecd_fos_2007_code, BlankEnum):
            oecd_fos_2007_code = (None, str(self.oecd_fos_2007_code.value).encode(), "text/plain")
        elif isinstance(self.oecd_fos_2007_code, None):
            oecd_fos_2007_code = (None, str(self.oecd_fos_2007_code).encode(), "text/plain")
        else:
            oecd_fos_2007_code = (None, str(self.oecd_fos_2007_code).encode(), "text/plain")
