...
from ...models.dashboard_my_order import DashboardMyOrder
    ...
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
    ...
    if response.status_code == 200:
        ...
        for response_200_item_data in _response_200:
            response_200_item = DashboardMyOrder.from_dict(response_200_item_data)
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[list["DashboardMyOrder"]]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """List current user's unfinished orders

     Returns the orders created by the current user that are still pending approval or being processed,
    newest first. Scoped to the requesting user only, so requesters can track their orders even without
    list permissions on the project or organization. Paginated: the exact total is in the X-Result-Count
    header.
...
def sync_all(
    *,
    ...
    Returns:
        list['DashboardMyOrder']: Combined results from all pages
    """
    from urllib.parse import parse_qs, urlparse

    all_results: list[DashboardMyOrder] = []
