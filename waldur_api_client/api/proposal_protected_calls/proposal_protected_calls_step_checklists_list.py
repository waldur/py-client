...
def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> list["ChecklistShort"]:
    ...
    if response.status_code == 200:
        ...
        for response_200_item_data in _response_200:
            response_200_item = ChecklistShort.from_dict(response_200_item_data)
...
def sync_detailed(
    *,
    ...
    """List checklists that can be attached to a workflow step (WORKFLOW_STEP-typed). Available to call
    managers so the workflow config UI can populate its checklist picker without staff-only access to
    the checklist admin API.
...
def sync_all(
    *,
    ...
    Returns:
        list['ChecklistShort']: Combined results from all pages
    """
    from urllib.parse import parse_qs, urlparse

    all_results: list[ChecklistShort] = []
